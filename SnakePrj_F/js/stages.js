...
class PlayStage extends Stage {
    constructor(gsm) {
        ...
        this.tete = new tete(CANVAS_WIDTH/2,CANVAS_HEIGHT/2,this.score)
        this.pomme =new pomme (this.tete,this.score)
    }
    ...
    update(dt)  {
        ...
        isGameOver() {
        if (this.tete.gameover == 1){
            if (this.score.isGameOver()) {
                    let options={
            name:this.score.getName(),
            points:this.score.getPoints()
            }
        this.gsm.changeStage(gStages.get("over"),options)}
    }
}
...
class OverStage extends Stage {
    ...
    render() {
		...
		//this.afficheName()                                              // afficher le nom 
        textSize(30)
        text('PRESS    ENTER    TO     RESTART', CANVAS_WIDTH/2,4*CANVAS_HEIGHT /5 )
        
        textSize(50)
        text("Game Over :" ,CANVAS_WIDTH/2,CANVAS_HEIGHT/2)
       
        textSize(50)
        text(this.score.getName()+ "        "+this.score.getPoints(),CANVAS_WIDTH/2,50+CANVAS_HEIGHT/2)
